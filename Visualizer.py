...
from ResistanceMap import ResistanceMap
...
class PIDVisualization:
  def __init__(self):
    ...
    self.standard_system = ResistanceMap(mass=1.0, base_friction=1)
    self.adaptive_system = ResistanceMap(mass=1.0, base_friction=1)
  ...
  def on_click(self, event):
    if event.inaxes == self.ax and event.button == 1:  # Left click
      ...
      self.standard_controller.setpoint = self.target_position
      self.adaptive_controller.setpoint = self.target_position
